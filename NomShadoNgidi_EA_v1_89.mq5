...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.89        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.89"
...
bool TryFVGBuy()
{
   ...
   // v1.88: previously skipped the whole setup outright when price had already run past
   // the structural TP before this hourly check could run ("TP already reached"). That
   // threw away genuine FVGs on fast moves instead of just re-targeting. Now falls
   // through to the same fallback TP calc used when no STH exists at all (tp <= ask
   // covers both cases), so a valid gap still gets traded with a recalculated target.
   if(tp <= ask)
   {
...
bool TryFVGSell()
{
   ...
   // the structural TP before this hourly check could run ("TP already reached"). That
   // threw away genuine FVGs on fast moves instead of just re-targeting. Now falls
   // through to the same fallback TP calc used when no STL exists at all, so a valid
   // gap still gets traded with a recalculated (further) target.
   if(tp <= 0 || tp >= bid)
   {
