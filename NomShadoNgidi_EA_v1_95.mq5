...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.95        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.95"
...
// FVG Buy (v1.93 rework)
// Candle 1 + candle 2 form the gap (candle 2's CLOSE clears candle 1's wick — see the
// scan loop below; the open doesn't matter, only where candle 2 settled). Two-tier entry:
//   RR >= 2.0 at current market price → enter now (waiting would only give a worse price)
//   RR <  2.0 at market → wait for a retrace into the candle1/candle2 gap zone and enter
//                          at its midpoint (Consequent Encroachment) via a resting limit
//                          order — skipped if RR at that zone entry is still < 1.5
// SL = candle 1's wick + buffer, TP = nearest structural swing high (fallback to 3R/Asian High).
bool TryFVGBuy()
{
   ...
   // v1.78: Faulty bullish FVG — candle 2 swept below candle 1 (liquidity grab before gap up)
   // Skip only this FVG; straight buys and other setups still proceed normally.
   if(iLow(_Symbol, PERIOD_H1, fvgBar + 1) < iLow(_Symbol, PERIOD_H1, fvgBar + 2))
   {
      PrintFormat("[FVG_Buy] SKIP: faulty FVG — bar[%d] low %.2f swept below bar[%d] low %.2f",
   ...
   // v1.91: two-tier entry. If RR already >= 2.0 at current market price, take it now —
   // don't wait, since a retrace to the FVG zone would only give a worse (lower) entry
   // for a buy. Only fall back to waiting for the zone if the market-price RR is weaker.
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   if(sl <= 0 || sl >= ask)
   { PrintFormat("[FVG_Buy] SKIP: invalid SL %.5f vs ask %.5f", sl, ask); return false; }

   double tpMarket = GetSTHigh(ask, InpSTH_Lookback);
   if(tpMarket <= ask)
   {
      double tp3R      = ask + 3.0 * (ask - sl);
      double tpAsianHi = (g_AsianHigh > ask) ? g_AsianHigh : 0;
      tpMarket = MathMax(tp3R, tpAsianHi);
   }
   // v1.94: mirror of FVG_Sell's all-bullish-Asian override — if every Asian session
   // candle was bearish and Asian Low still got broken (this setup wouldn't even be
   // running otherwise), the whole session was a one-way sell-off with no real resistance
   // structure above; use the Asian High as TP instead of a structural swing high.
   if(g_AllAsianBearish && g_AsianHigh > ask)
      tpMarket = g_AsianHigh;

   double rrrMarket = (tpMarket - ask) / (ask - sl);
   if(rrrMarket >= 2.0)
   {
      PrintFormat("[FVG_Buy] RR %.2f >= 2.0 at market — entering immediately at %.5f (sl=%.5f tp=%.5f)",
                  rrrMarket, ask, sl, tpMarket);
      bool ok = PlaceBuy(ask, sl, tpMarket, "FVG_Buy");
      if(ok) g_FVGBuyDone = true;
      return ok;
   }

   // v1.90: market RR wasn't strong enough — wait for price to retrace back into the
   // candle1/candle2 gap zone (the actual FVG) and enter at its midpoint (Consequent
   // Encroachment) via a resting limit order, instead of chasing a worse market price.
   // v1.93: zone's upper bound is candle 2's close (matching the close-based gap check),
   // not its body bottom.
   double zoneLow  = iHigh(_Symbol, PERIOD_H1, fvgBar + 1);   // candle 1 high
   double zoneHigh = iClose(_Symbol, PERIOD_H1, fvgBar);      // candle 2 close
   double entry    = (zoneHigh + zoneLow) / 2.0;                                                  // CE — 50% of the gap
   PrintFormat("[FVG_Buy] Market RR %.2f < 2.0 — FVG zone %.5f–%.5f, entry (CE) at %.5f",
               rrrMarket, zoneLow, zoneHigh, entry);

   if(sl <= 0 || sl >= entry)
   { PrintFormat("[FVG_Buy] SKIP: invalid SL %.5f vs zone entry %.5f", sl, entry); return false; }
   ...
   double rrrZone = (tp - entry) / (entry - sl);
   if(rrrZone < 1.5)
   {
      PrintFormat("[FVG_Buy] SKIP: RR %.2f < 1.5 at zone entry %.5f — insufficient quality", rrrZone, entry);
      return false;
   }

   PrintFormat("[FVG_Buy] RR %.2f — placing order at zone entry %.5f (sl=%.5f tp=%.5f)", rrrZone, entry, sl, tp);
...
//                          at its midpoint (Consequent Encroachment) via a resting limit
//                          order — skipped if RR at that zone entry is still < 1.5
// SL = candle 1's wick + buffer, TP = nearest structural swing low (fallback to 3R/Asian Low).
bool TryFVGSell()
{
   if(g_DailyCount >= InpMaxDailyTrades) return false;
   ...
   // v1.78: Faulty bearish FVG — candle 2 swept above candle 1 (liquidity grab before gap down)
   // Skip only this FVG; straight sells and other setups still proceed normally.
   if(iHigh(_Symbol, PERIOD_H1, fvgBar + 1) > iHigh(_Symbol, PERIOD_H1, fvgBar + 2))
   {
      PrintFormat("[FVG_Sell] SKIP: faulty FVG — bar[%d] high %.2f swept above bar[%d] high %.2f",
   ...
   // v1.91: two-tier entry. If RR already >= 2.0 at current market price, take it now —
   // don't wait, since a retrace to the FVG zone would only give a worse (higher) entry
   // for a sell. Only fall back to waiting for the zone if the market-price RR is weaker.
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(sl <= bid)
   { PrintFormat("[FVG_Sell] SKIP: invalid SL %.5f vs bid %.5f", sl, bid); return false; }

   double tpMarket = GetSTLow(bid, InpSTH_Lookback);
   if(tpMarket <= 0 || tpMarket >= bid)
   {
      double tp3R      = bid - 3.0 * (sl - bid);
      double tpAsianLo = (g_AsianLow > 0 && g_AsianLow < bid) ? g_AsianLow : tp3R;
      tpMarket = MathMin(tp3R, tpAsianLo);
   }
   if(g_AllAsianBullish && g_AsianLow > 0 && g_AsianLow < bid)
      tpMarket = g_AsianLow;

   double rrrMarket = (bid - tpMarket) / (sl - bid);
   if(rrrMarket >= 2.0)
   {
      PrintFormat("[FVG_Sell] RR %.2f >= 2.0 at market — entering immediately at %.5f (sl=%.5f tp=%.5f)",
                  rrrMarket, bid, sl, tpMarket);
      bool ok = PlaceSell(bid, sl, tpMarket, "FVG_Sell");
   ...
   // candle1/candle2 gap zone (the actual FVG) and enter at its midpoint (Consequent
   // Encroachment) via a resting limit order, instead of chasing a worse market price.
   // v1.93: zone's lower bound is candle 2's close (matching the close-based gap check),
   // not its body top.
   double zoneHigh = iLow(_Symbol, PERIOD_H1, fvgBar + 1);    // candle 1 low
   double zoneLow  = iClose(_Symbol, PERIOD_H1, fvgBar);      // candle 2 close
   double entry    = (zoneHigh + zoneLow) / 2.0;                                                  // CE — 50% of the gap
   PrintFormat("[FVG_Sell] Market RR %.2f < 2.0 — FVG zone %.5f–%.5f, entry (CE) at %.5f",
               rrrMarket, zoneLow, zoneHigh, entry);

   if(sl <= entry)
   { PrintFormat("[FVG_Sell] SKIP: invalid SL %.5f vs zone entry %.5f", sl, entry); return false; }
   ...
   double rrrZone = (entry - tp) / (sl - entry);
   if(rrrZone < 1.5)
   {
      PrintFormat("[FVG_Sell] SKIP: RR %.2f < 1.5 at zone entry %.5f — insufficient quality", rrrZone, entry);
      return false;
   }

   PrintFormat("[FVG_Sell] RR %.2f — placing order at zone entry %.5f (sl=%.5f tp=%.5f)", rrrZone, entry, sl, tp);
