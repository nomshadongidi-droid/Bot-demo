...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.83        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.83"
...
// v1.83: minimum lot step enforced on top of the broker's own step, so lots
// never round finer than the broker allows. BlackBull only fills 0.1 increments;
// Vault Markets accepts 0.01 — set this to 0.01 on the Vault Markets account.
input double InpMinLotStep     = 0.1;  // Minimum Lot Step (BlackBull=0.1, Vault Markets=0.01)
...
void CheckScaleIn()
{
   ...
   double effectiveStep = MathMax(SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP), InpMinLotStep);
...
double CalcLotSize(double slPips)
{
   ...
   // enforce InpMinLotStep on top of the broker's own step (BlackBull needs 0.1;
   // set InpMinLotStep=0.01 on the Vault Markets account for 2-decimal lots)
   double effectiveStep = MathMax(step, InpMinLotStep);
   int    lotDigits     = LotStepDigits(effectiveStep);
