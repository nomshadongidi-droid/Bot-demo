...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.90        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.90"
...
// FVG Buy (v1.90 rework)
// Candle 1 + candle 2 form the gap (candle 2's body clears candle 1's wick — see the
// scan loop below). Entry is NOT at current market price: it's a limit order resting at
// the midpoint (Consequent Encroachment) of that gap zone, so the trade only fills if
// price actually retraces back into the FVG. SL = candle 1's wick + buffer, TP = nearest
// structural swing high (fallback to 3R/Asian High). Skipped if RR at the zone entry < 1.5.
bool TryFVGBuy()
{
   ...
   // v1.90: entry no longer chases current ask — it now waits for price to retrace back
   // into the candle1/candle2 gap zone (the actual FVG) and enters at the zone's midpoint
   // (Consequent Encroachment), via a resting limit order. Candle 3 (whatever candle is
   // current when this fires) is purely the moment of entry, not part of the gap itself.
   ...
   PrintFormat("[FVG_Buy] FVG zone %.5f–%.5f — entry (CE) at %.5f", zoneLow, zoneHigh, entry);

   double sl = iLow(_Symbol, PERIOD_H1, fvgBar + 1) - PipsToPrice(InpFVGBuffer);
   double tp = GetSTHigh(entry, InpSTH_Lookback);

   if(sl <= 0 || sl >= entry)
   { PrintFormat("[FVG_Buy] SKIP: invalid SL %.5f vs entry %.5f", sl, entry); return false; }

   // No STH found, or it sits at/below the planned entry — recalc a fresh, further target
   if(tp <= entry)
   {
   ...
   double rrr = (tp - entry) / (entry - sl);
   if(rrr < 1.5)
   {
      PrintFormat("[FVG_Buy] SKIP: RR %.2f < 1.5 at zone entry %.5f — insufficient quality", rrr, entry);
      return false;
   }

   PrintFormat("[FVG_Buy] RR %.2f — placing order at zone entry %.5f (sl=%.5f tp=%.5f)", rrr, entry, sl, tp);
...
// FVG Sell (v1.90 rework)
// Candle 1 + candle 2 form the gap (candle 2's body clears candle 1's wick — see the
// scan loop below). Entry is NOT at current market price: it's a limit order resting at
// the midpoint (Consequent Encroachment) of that gap zone, so the trade only fills if
// price actually retraces back into the FVG. SL = candle 1's wick + buffer, TP = nearest
// structural swing low (fallback to 3R/Asian Low). Skipped if RR at the zone entry < 1.5.
bool TryFVGSell()
{
   if(g_DailyCount >= InpMaxDailyTrades) return false;
   ...
   // v1.90: entry no longer chases current bid — it now waits for price to retrace back
   // into the candle1/candle2 gap zone (the actual FVG) and enters at the zone's midpoint
   // (Consequent Encroachment), via a resting limit order. Candle 3 (whatever candle is
   // current when this fires) is purely the moment of entry, not part of the gap itself.
   ...
   double entry    = (zoneHigh + zoneLow) / 2.0;                                                  // CE — 50% of the gap
   PrintFormat("[FVG_Sell] FVG zone %.5f–%.5f — entry (CE) at %.5f", zoneLow, zoneHigh, entry);

   double sl = iHigh(_Symbol, PERIOD_H1, fvgBar + 1) + PipsToPrice(InpFVGBuffer);
   double tp = GetSTLow(entry, InpSTH_Lookback);

   if(sl <= entry)
   { PrintFormat("[FVG_Sell] SKIP: invalid SL %.5f vs entry %.5f", sl, entry); return false; }

   // No STL found, or it sits at/above the planned entry — recalc a fresh, further target
   if(tp <= 0 || tp >= entry)
   {
   ...
   double rrr = (entry - tp) / (sl - entry);
   if(rrr < 1.5)
   {
      PrintFormat("[FVG_Sell] SKIP: RR %.2f < 1.5 at zone entry %.5f — insufficient quality", rrr, entry);
      return false;
   }

   PrintFormat("[FVG_Sell] RR %.2f — placing order at zone entry %.5f (sl=%.5f tp=%.5f)", rrr, entry, sl, tp);
