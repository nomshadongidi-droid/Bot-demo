...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.92        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.92"
...
// FVG Buy (v1.91 rework)
// Candle 1 + candle 2 form the gap (candle 2's body clears candle 1's wick — see the
// scan loop below). Two-tier entry:
...
bool TryFVGBuy()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.89: reverted to body-vs-wick (was body-vs-body in v1.81-v1.88). FVG = body of
      // candle 3 entirely above candle 2's HIGH (its wick extreme), not just its body top.
      // v1.81 required clearing candle 2's whole body, which is a stricter bar than the
      // standard "gap vs the previous candle's wick" definition and was missing real setups.
      double c3BodyBot = MathMin(iOpen(_Symbol, PERIOD_H1, i), iClose(_Symbol, PERIOD_H1, i));
      double c2High    = iHigh(_Symbol, PERIOD_H1, i + 1);

      if(c3BodyBot > c2High)
      {
         fvgBar = i;
         PrintFormat("[FVG_Buy] Bullish FVG at bar[%d] %02d:00 ET — body bottom %.2f > bar[%d] high %.2f (gap %.2f pts)",
                     i, bDt.hour, c3BodyBot, i+1, c2High, c3BodyBot - c2High);
         break;
      }
      else
      {
         PrintFormat("[FVG_Buy] bar[%d] %02d:00 ET: body bottom=%.2f bar[%d] high=%.2f gap=%.2f — no FVG",
                     i, bDt.hour, c3BodyBot, i+1, c2High, c3BodyBot - c2High);
      }
   }
   ...
   double zoneHigh = MathMin(iOpen(_Symbol, PERIOD_H1, fvgBar), iClose(_Symbol, PERIOD_H1, fvgBar)); // candle 2 body bottom
...
bool TryFVGSell()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.89: reverted to body-vs-wick (was body-vs-body in v1.81-v1.88). FVG = body of
      // candle 3 entirely below candle 2's LOW (its wick extreme), not just its body bottom.
      // v1.81 required clearing candle 2's whole body, which is a stricter bar than the
      // standard "gap vs the previous candle's wick" definition and was missing real setups.
      double c3BodyTop = MathMax(iOpen(_Symbol, PERIOD_H1, i), iClose(_Symbol, PERIOD_H1, i));
      double c2Low     = iLow(_Symbol, PERIOD_H1, i + 1);

      if(c3BodyTop < c2Low)
      {
         fvgBar = i;
         PrintFormat("[FVG_Sell] Bearish FVG at bar[%d] %02d:00 ET — body top %.2f < bar[%d] low %.2f (gap %.2f pts)",
                     i, bDt.hour, c3BodyTop, i+1, c2Low, c2Low - c3BodyTop);
         break;
      }
      else
      {
         PrintFormat("[FVG_Sell] bar[%d] %02d:00 ET: body top=%.2f bar[%d] low=%.2f gap=%.2f — no FVG",
                     i, bDt.hour, c3BodyTop, i+1, c2Low, c3BodyTop - c2Low);
      }
   }
   ...
   double zoneLow  = MathMax(iOpen(_Symbol, PERIOD_H1, fvgBar), iClose(_Symbol, PERIOD_H1, fvgBar)); // candle 2 body top
   double entry    = (zoneHigh + zoneLow) / 2.0;                                                  // CE — 50% of the gap
