...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.77        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.77"
...
int OnInit()
{
   ...
   PrintFormat("=== Nomshado Ngidi EA v1.77 Initialised ===");
...
bool CheckFaultyBullishFVG()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // Faulty sweep detection uses 3-candle wick definition (broader than trade-entry FVG)
      // — detects the structural liquidity grab pattern regardless of body gap
      ...
      if(c3Low > c1High)
      {
         ...
         PrintFormat("[FaultyCheck] Bullish sweep pattern at bar[%d] %02d:00 ET — faulty=%s (candle2 low %.2f vs candle1 low %.2f)",
...
bool CheckFaultyBearishFVG()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      if(c3High < c1Low)
      {
         ...
         PrintFormat("[FaultyCheck] Bearish sweep pattern at bar[%d] %02d:00 ET — faulty=%s (candle2 high %.2f vs candle1 high %.2f)",
                     i, bDt.hour, faulty?"YES":"NO",
...
bool TryFVGBuy()
{
   ...
   // v1.73: Faulty bullish FVG — candle 2 (bar[fvgBar+1]) swept below candle 1 (bar[fvgBar+2])
