...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.86        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.86"
...
bool ScanBuySetups()
{
   ...
   // v1.86: gate widened from "hr < InpNYKillZoneNY" to "hr <= InpNYKillZoneNY" — the 5am
   // candle only finishes closing (becomes bar[1]) at the hr=6 tick, so the old exclusive
   // bound meant the last London candle could never be evaluated at all. This still only
   // ever reads bar[1] as the just-closed 5am candle (a London-hour candle) — it does not
   // let TryFVGBuy see any 6am-or-later (NY session) candle data.
