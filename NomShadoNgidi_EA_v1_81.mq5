...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.81        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.81"
...
int OnInit()
{
   ...
   PrintFormat("=== Nomshado Ngidi EA v1.81 Initialised ===");
...
// Returns the nearest bull→bear body transition whose open is ABOVE minPrice (entry price).
// v1.79: changed filter from > g_AsianHigh to > minPrice so that pre-Asian swing highs
// below the Asian High are not wrongly discarded. 3R fallback only fires when truly no
// pre-Asian price history exists above current entry (e.g. market at all-time highs).
double GetSTHigh(double minPrice, int lookback = 20)
{
   ...
   for(int i = 1; i <= lim; i++)
   {
      ...
      if(IsBearishCandle(i) && IsBullishCandle(i + 1))
      {
         ...
         if(level > minPrice)
         {
            PrintFormat("[GetSTHigh] Bull→Bear above entry at bar[%d] (pre-Asian): %.5f", i, level);
            return level;
         }
      }
   }

   PrintFormat("[GetSTHigh] No valid STH found above entry before Asian session");
   return 0;
}

// GetSTLow — TP for sell setups.
// Scans backwards (most-recent first) through bars BEFORE today's Asian session start.
// Returns the nearest bear→bull body transition whose open is BELOW maxPrice (entry price).
// v1.79: changed filter from < g_AsianLow to < maxPrice so that pre-Asian swing lows
// above the Asian Low are not wrongly discarded. 3R fallback only fires when truly no
// pre-Asian price history exists below current entry.
double GetSTLow(double maxPrice, int lookback = 20)
{
   ...
   for(int i = 1; i <= lim; i++)
   {
      ...
      if(IsBullishCandle(i) && IsBearishCandle(i + 1))
      {
         ...
         if(level < maxPrice)
         {
            PrintFormat("[GetSTLow] Bear→Bull below entry at bar[%d] (pre-Asian): %.5f", i, level);
            return level;
         }
      }
   }

   PrintFormat("[GetSTLow] No valid STL found below entry before Asian session");
   return 0;
}
