...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.101       |
//+------------------------------------------------------------------+
//
...
//
//  v1.101 CHANGES (from v1.100):
//  • Removed TryAsianSellReentryBuy and its CheckAsianSellSLReentry trigger entirely —
//    the "re-enter BUY after FVG_Asian_Sell's SL is hit" mechanism is gone, along with
//    the g_AsianSellSLHit/g_AsianSellReentered state it used. The six setup models listed
//    above are now the complete list — nothing implicit runs outside them.
//
//  v1.100 CHANGES (from v1.99):
//  • New input InpTPBufferPips (default 2). Every setup's TP is now pulled this many pips
//    closer to entry than the structurally-computed target — applied after all TP
//    fallback/override logic (STH/STL, 3R fallback, all-Asian override, D1 reversal
//    level) but before the trade is placed, on all six setups: FVG Buy/Sell, Straight
//    Buy/Sell, FVG Asian Buy/Sell. Gives the trade a small cushion instead of needing the
//    exact structural level to be tagged.
//
//  v1.99 CHANGES (from v1.98):
//  • GetMinLotStep: AvaTrade's ACCOUNT_COMPANY string is "Ava Trade Markets Ltd." — two
//    words, with a space — so the existing "avatrade" (one word) substring check never
//    matched, and every AvaTrade account was silently defaulting to the 0.1 lot step
//    instead of the 0.01 it actually supports. Lots were landing only on 0.1 increments
//    (1.20, 1.10, ...), never a real second decimal digit. Now checks both spellings.
//
//  v1.98 CHANGES (from v1.96):
//  • TryFVGBuy / TryFVGSell: redefined the faulty-FVG liquidity-sweep filter. It used to
//    compare the candle right before the gap candle against an even older third candle
//    (candle 1) — that third candle is removed from the check entirely now. The filter
//    now only ever looks at the gap candle itself against the candle right before it:
//    for a bullish FVG, faulty if the gap candle's own low dips below the prior candle's
//    low; for a bearish FVG, faulty if the gap candle's own high climbs above the prior
//    candle's high. This check now only applies when the gap candle closed between
//    02:00-04:00 ET — the same dip/climb at 05:00 ET or later no longer disqualifies the
//    FVG. Confirmed live: a genuine bullish FVG at the 05:00 ET candle was being rejected
//    under the old candle-1-based rule even though the actual gap (05:00 candle's close
//    vs the 04:00 candle's high) was valid; under this rule it now passes.
//
//  v1.96 CHANGES (from v1.95):
//  • TryFVGBuy / TryFVGSell: removed the v1.90/v1.91 "wait for a retrace into the FVG
//    zone" entry mechanism entirely, reverting to the original, confirmed-correct 3-tier
//    entry structure:
...
//                         (entry12 = (tp + 2*sl)/3) — not tied to the FVG zone itself
//      RR <  1.5 → skip
//    Everything else built on top since then is unchanged and still applies on top of
//    this entry logic: the v1.93 close-based gap detection, the v1.78 faulty-FVG sweep
//    filter, the v1.92 bounded GetSTHigh/GetSTLow lookback, and the v1.94 all-Asian TP
//    overrides.
//
//  v1.95 CHANGES (from v1.94):
//  • OnInit: removed the v1.47 "scan immediately on init" block. It called
//    ScanBuySetups/ScanSellSetups right when the EA was attached (or restarted),
//    evaluating whatever H1 candle had already closed BEFORE the EA was ever running —
//    so attaching mid-session could immediately fire a trade off a setup that's actually
//    hours old, from a move that already substantially played out. Confirmed live: EA
//    attached, Straight_Sell fired within minutes off a bar[1] close from well before
//    attachment. The EA now always waits for a new H1 bar to close while it's actually
//    running (the existing g_LastBar check in OnTick) before evaluating or acting on any
//    setup — whether freshly attached or restarted. g_AsianHigh/g_AsianLow are still
//    computed on init (AnalyseAsianSession) so direction bias is known immediately; only
//    the immediate trade-firing scan was removed.
//
//  v1.94 CHANGES (from v1.93):
//  • Added the missing buy-side mirror of the all-bullish-Asian TP override. TryFVGSell
//    (both entry tiers) and TryStraightSell already override TP to the Asian Low when
//    every Asian session candle was bullish and price still broke the Asian High — a
//    one-way sell-off day with no real resistance structure above. g_AllAsianBearish was
//    computed the whole time but never actually read anywhere. TryFVGBuy (both entry
//    tiers) and TryStraightBuy now apply the same logic in reverse: if every Asian
//    session candle was bearish and Asian Low still broke, TP is overridden to the Asian
//    High instead of a structural swing high. All four setup functions are symmetric now.
//
//  v1.93 CHANGES (from v1.92):
//  • TryFVGBuy / TryFVGSell: gap check changed from candle 2's body-top/bottom
//    (MathMax/MathMin of open+close) to candle 2's CLOSE. A candle that opens back
//    inside candle 1's range and only closes beyond it — e.g. bearish case: opens above
//    candle 1's low, closes below it — was being rejected because its OPEN (its body
//    top) never cleared candle 1's low, even though the zone below candle 1's low sat
//    untouched right up until candle 2's close. Only the close (where price actually
//    settled) determines whether that zone got filled — the open is irrelevant to it.
//  • Zone-entry (CE) calc for the v1.90 retrace-limit-order path updated to match: bounded
//    by candle 2's close instead of its body top/bottom.
//  • NOTE: the separate faulty-FVG liquidity-sweep filter (v1.78, unchanged) can still
//    reject an otherwise-valid gap under this new rule if candle 1 itself swept above/
//    below candle 0's high/low — that's an intentional, different risk filter, not part
//    of this change.
//
//  v1.92 CHANGES (from v1.91):
//  • GetSTHigh / GetSTLow: fixed a real bug — both took a `lookback` parameter (default
//    20 bars) that was never actually used to bound the search. `lim` was set to nearly
//    the entire available price history instead, so when no qualifying swing existed
//    recently, the scan kept walking backward through weeks of candles until it found
//    ANY match, however stale. Confirmed live: a Straight_Buy TP landed on a swing high
//    from over a week earlier, producing a meaningless RRR of 16.45 that sailed straight
//    through the RR >= 2.0 fast-path check. Both now use `MathMin(lookback, iBars-2)`,
//    matching GetSTHighForSL/GetSTLowForSL, which already bounded correctly.
//  • Investigated the same trade for a directional-bias bug (bought during what looked
//    like a clear multi-week downtrend) — AsianBreakDirection() only computes its result
//    from g_AsianHigh/g_AsianLow, which AnalyseAsianSession() sets once per day and never
//    recomputes (`if(today == g_AsianDate) return;`), so it can't flip mid-day. That part
//    was working as designed; the bad TP above was the actual bug making the trade look
//    wrong.
//
//  v1.91 CHANGES (from v1.90):
//  • TryFVGBuy / TryFVGSell: restored the market-entry fast path on top of v1.90's zone
//    entry. v1.90 made every FVG entry wait for a retrace into the gap zone, even when
//    RR was already >= 2.0 at current market price — but waiting in that case only gives
//    a worse entry (higher for a sell, lower for a buy) for no benefit, and risks missing
//    the move if price never comes back. Now: RR >= 2.0 at market → enter immediately;
//    otherwise → fall back to v1.90's zone-midpoint limit order (skipped if RR there is
//    still < 1.5). Both RR checks and TP calcs are computed independently against their
//    own entry price (market bid/ask vs the zone's CE) since they can differ.
//
//  v1.90 CHANGES (from v1.89):
//  • TryFVGBuy / TryFVGSell: reworked entry mechanics entirely. Previously entered at
//    current market price (chasing whatever bid/ask was when the RR check passed) or a
//    price solved to hit exactly 1:2 RR. Neither waited for price to actually return to
//    the FVG. Now computes the real gap zone (candle 1's wick to candle 2's body) and
//    places a resting limit order at the zone's midpoint (Consequent Encroachment) — the
//    trade only fills if price genuinely retraces into the gap, matching how FVG entries
//    are meant to work. Existing pending-order dispatch in PlaceBuy/PlaceSell (which
//    already picks Limit vs Stop based on entry vs current price) needed no changes.
//    RR is now checked at the zone entry price, not current price, and the whole tiered
//    "market if RR>=2 / solve for exact 1:2 / skip" cascade is replaced by one check:
//    skip if RR at the zone entry is below 1.5.
//
//  v1.89 CHANGES (from v1.88):
//  • TryFVGBuy / TryFVGSell: reverted the core gap comparison from body-vs-body (v1.81)
//    back to body-vs-wick (v1.76-v1.80's definition). v1.81 required candle 3's body to
//    clear candle 2's entire BODY; this restores clearing candle 2's wick extreme
//    (high for sell, low for buy) instead — a looser, more standard bar that catches
//    real setups v1.81 was silently rejecting (confirmed via git history: v1.81's own
//    commit message describes this as an intentional tightening, not a fix for a bug).
//    DetectFVG()/GetFVGZone() (used by FVG Asian Buy/Sell) already used this definition
//    and were never changed — the codebase is now consistent again across all four setups.
//
//  v1.88 CHANGES (from v1.87):
//  • TryFVGBuy / TryFVGSell: removed the outright "TP already reached" SKIP. On a fast
//    move, price can blow past the structural TP (STHigh/STLow) in the time between the
//    FVG forming and the next hourly check — the setup was being thrown away entirely
//    instead of just re-targeted. Now both fall through to the same fallback TP calc
//    already used when no STH/STL exists (3R vs Asian High/Low), so a genuine FVG still
//    gets traded with a recalculated target instead of being skipped.
//
//  v1.87 CHANGES (from v1.86):
//  • ScanBuySetups: FVG Buy's NY-session restriction removed. It previously stopped
//    scanning at hr <= InpNYKillZoneNY (6) — now scans the full London + NY KZ window
//    (hr < InpTradingEndNY, i.e. 02:00-10:00), same as FVG Sell already did. This
//    subsumes v1.86's 5am-candle fix (hr=6 is inside the wider range too).
//  • Looked into also widening IsInTradingWindow()/FVG Sell to catch the 9am candle
//    (the sell-side mirror of v1.86's bug) but did NOT make that change: the file's own
//    v1.12 changelog documents that exact boundary (hr <= InpTradingEndNY →
//    hr < InpTradingEndNY, across IsInTradingWindow + both Scan*Setups) as a CRITICAL
//    fix specifically to stop new trades opening at the 10:00 bar. Widening it back
//    would reopen that. Left as-is pending an explicit decision to trade off "FVG Sell
//    sees the 9am candle" against "no new entries fire exactly at 10:00."
//
//  v1.86 CHANGES (from v1.85):
//  • ScanBuySetups: FVG Buy's hour gate widened from "hr < InpNYKillZoneNY" to
//    "hr <= InpNYKillZoneNY". The 5am ET candle only finishes closing (becomes bar[1])
//    at the hr=6 tick, so the old exclusive bound meant FVG Buy could NEVER see the last
//    London-hour candle — Straight Buy would always catch it first instead, since its own
//    gate already included hr=6. Now FVG Buy gets first look at the 5am candle before
//    Straight Buy does, matching the documented 02:00-06:00 intent. Does not let FVG Buy
//    see any 6am-or-later (NY session) candle — that restriction is unchanged.
//  • NOTE: the identical gap exists on the sell side — FVG Sell can never evaluate the
//    9am candle, because IsInTradingWindow() (the shared OnTick gate for ALL setups)
//    cuts off at hr < InpTradingEndNY (10). Not fixed here since it touches a boundary
//    shared by Straight Buy/Sell as well — left for a follow-up once confirmed wanted.
//
//  v1.85 CHANGES (from v1.84):
//  • GetMinLotStep: added AvaTrade to the broker auto-detect list — accounts whose
//    ACCOUNT_COMPANY contains "AvaTrade" now also get 0.01 lot rounding, same as
//    Vault Markets. BlackBull (no match) stays on 0.1.
//
//  v1.84 CHANGES (from v1.83):
//  • v1.83's InpMinLotStep required manually editing an EA input on the Vault Markets
//    chart — easy to miss, and lots kept rounding to 0.1 when it wasn't set. Replaced
//    with GetMinLotStep(), which reads AccountInfoString(ACCOUNT_COMPANY) and picks
//    0.01 automatically when the broker name contains "Vault", else 0.1 (BlackBull).
//    No input needs to be touched — same .mq5 file works correctly on both accounts.
//  • Renamed the (now optional) input to InpMinLotStepOverride, default 0 = auto-detect.
//    Set it to a positive value only to force a specific step regardless of broker.
//
//  v1.83 CHANGES (from v1.82):
//  • Added InpMinLotStep input (default 0.1, unchanged for BlackBull). Lot sizing no longer
//    hardcodes a 0.1 rounding step — it now rounds to MathMax(broker step, InpMinLotStep).
//    Set InpMinLotStep=0.01 on the Vault Markets account to get lots rounded to 2 decimal
//    places (e.g. 0.17) instead of 1 (0.2). CalcLotSize and CheckScaleIn both updated.
//
//  v1.82 CHANGES (from v1.81):
//  • GetSTHigh: TP filter raised from > entry to > MathMax(entry, g_AsianHigh).
//    The Asian session sweeps through all pre-Asian levels below the Asian High, so any
//    STH in that zone is already consumed and invalid as a TP target. By requiring
//    level > Asian High, the scan skips those and finds the first unswept resistance above
//    the Asian High. 3R fallback still fires if no such level exists.
//  • GetSTLow: same fix — filter lowered to MathMin(entry, g_AsianLow).
//    Only pre-Asian support BELOW the Asian Low (unswept) is used as TP for sell setups.
//
//  v1.81 CHANGES (from v1.80):
//  • FVG Buy / FVG Sell: Changed FVG detection from body-vs-wick to body-vs-body comparison.
//    Bullish FVG: c3 body bottom > c2 BODY TOP (was: c2 HIGH wick).
//    Bearish FVG: c3 body top < c2 BODY BOTTOM (was: c2 LOW wick).
//    Rationale: When c2 (01:00 bar) has a long wick above a small body, the old wick-based
//    check rejected valid FVGs where c3 body sits above c2 body but below c2 wick. The
//    body-vs-body check captures the real price gap between candle bodies (the true FVG).
//  • Also reverted v1.80's hour < 1 extension back to hour < 2 — the missed FVG was between
//    the 01:00 bar (c2) and 02:00 bar (c3), which hour < 2 already covers as the scan looks
//    back from current bar. The issue was the wick-based detection, not the scan range.
//
//  v1.80 CHANGES (from v1.79):
//  • FVG Buy / FVG Sell: FVG scan extended to include 01:00 ET bars (changed hour < 2 to hour < 1)
//    [REVERTED in v1.81 — see above]
//
//  v1.79 CHANGES (from v1.78):
//  • GetSTHigh: changed TP filter from > g_AsianHigh to > entry price (minPrice param)
//    Previously, any pre-Asian swing high below the Asian High was silently discarded,
//    causing GetSTHigh to return 0 even when valid structural targets existed. This made
//    the 3R fallback fire on days where the Asian session had consumed all the nearby
//    swing highs (Asian High above recent STHs), producing trades with no structural TP.
//    Fix: scan pre-Asian bars for nearest bull→bear transition above the actual entry price.
//    The 3R fallback is kept — it only fires now when price is genuinely at all-time highs
//    with no pre-Asian price history above entry.
//  • GetSTLow: same fix — filter changed from < g_AsianLow to < entry price (maxPrice param)
//  • All callers updated: GetSTHigh(ask/entry, ...) and GetSTLow(bid/entry, ...)
//  • When STH/STL found but RRR < 1.5 → trade is skipped (no 3R override);
//    the 3R fallback only applies when truly no structural level exists.
//
//  v1.78 CHANGES (from v1.77):
//  • Faulty sweep logic simplified: a faulty FVG now only cancels that specific FVG setup,
//    it no longer flips the overall direction or blocks all other setups (Straight Buy/Sell).
//    Previous behaviour: faulty FVG → set g_FaultyLowSweep/g_FaultyHighSweep → block ALL
//    buy/sell setups + flip direction in AsianBreakDirection() → missed valid opportunities.
//    New behaviour: faulty FVG found in TryFVGBuy/TryFVGSell → return false (skip only
//    that FVG), all other setups (straight buys/sells with other FVGs) proceed normally.
//  • Removed: g_FaultyLowSweep, g_FaultyHighSweep global flags
//  • Removed: CheckFaultyBullishFVG(), CheckFaultyBearishFVG() helper functions
//  • Removed: faulty-sweep override in AsianBreakDirection(); highAlsoRan/lowAlsoRan tracking
//    also removed (no longer needed). v1.74 both-levels-same-bar close-based logic retained.
//  • Removed: block guards from TryStraightBuy and TryStraightSell
//
//  v1.77 CHANGES (from v1.76):
//  • Fixed: faulty sweep detection (CheckFaultyBullishFVG/CheckFaultyBearishFVG) reverted
//    to 3-candle wick definition. v1.76 changed these to body-gap which broke detection
//    for today's pattern — no bar had its body fully beyond the previous bar's extreme,
//    so g_FaultyLowSweep stayed false and a BUY fired instead of the correct SELL.
//    Two separate definitions now deliberately used:
//      Trade entry FVG (TryFVGBuy/TryFVGSell): 2-candle body-gap (user's definition)
//      Faulty sweep detection (CheckFaulty*): 3-candle wick (structural liquidity grab)
//
//  v1.76 CHANGES (from v1.75):
//  • FVG detection reworked to correct definition (buy and sell):
//    Old (wrong): 3-candle wick-to-wick — candle3.HIGH < candle1.LOW (bearish)
//    New (correct): 2-candle body-gap — candle3.BODY_TOP < candle2.LOW (bearish)
//                                       candle3.BODY_BOTTOM > candle2.HIGH (bullish)
//    The FVG is the gap between candle 2's extreme (low/high) and candle 3's body.
//    Candle 3's body must be entirely beyond candle 2's low (sell) or high (buy).
//  • Same fix applied to CheckFaultyBullishFVG() and CheckFaultyBearishFVG()
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.101"
...
// v1.84: minimum lot step is now auto-detected from the broker name (ACCOUNT_COMPANY) —
// BlackBull gets 0.1, Vault Markets gets 0.01, no input needs to be touched. Leave this
// at 0 for auto-detect; set a positive value (e.g. 0.01) only to force a specific step.
input double InpMinLotStepOverride = 0;    // Minimum Lot Step Override (0 = auto-detect by broker)

input group "=== Stop Loss Settings ==="
input int    InpFVGBuffer      = 5;    // SL/entry buffer in pips

input group "=== Take Profit Settings ==="
// v1.100: TP pulled in this many pips closer to entry than the structurally-computed
// target, on every setup (FVG Buy/Sell, Straight Buy/Sell, FVG Asian Buy/Sell) — applied
// after all TP fallback/override logic, before the RR gate, so RR is calculated on the
// same buffered target that actually gets placed.
input int    InpTPBufferPips   = 2;    // Pips to pull TP in from the original target
...
int OnInit()
{
   ...
   PrintFormat("=== Nomshado Ngidi EA v1.82 Initialised ===");
   ...
   // v1.95: removed the v1.47 "scan immediately on init" behavior. It fired on whatever
   // candle had already closed before the EA was ever attached — meaning attaching (or
   // restarting) mid-session could immediately enter a trade off a setup that's actually
   // hours old and already substantially played out, rather than a fresh signal. The EA
   // now always waits for a genuinely new H1 bar to close while it's running (the normal
   // g_LastBar check in OnTick) before it will ever evaluate or act on any setup, whether
   // freshly attached or restarted mid-session.
   ResetDailyCount();
   AnalyseAsianSession();
...
void CheckScaleIn()
{
   ...
   double effectiveStep = MathMax(SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP), GetMinLotStep());
   double minLot        = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   int    lotDigits     = LotStepDigits(effectiveStep);

   // How many more lots are still needed
   double neededLots = NormalizeDouble(
      MathRound((g_TargetLots - totalLots) / effectiveStep) * effectiveStep, lotDigits);
   ...
   double maxAffordable = NormalizeDouble(
      MathFloor((freeMargin * 0.9) / marginPerLot / effectiveStep) * effectiveStep, lotDigits);
   if(maxAffordable < minLot) return;  // not enough margin yet

   double addLots = NormalizeDouble(
      MathRound(MathMin(neededLots, maxAffordable) / effectiveStep) * effectiveStep, lotDigits);
   if(addLots < minLot) return;

   PrintFormat("[ScaleIn] Open=%.2f Target=%.2f Adding=%.2f (freeMargin=%.2f marginPerLot=%.2f)",
   ...
   if(ok)
      PrintFormat("[ScaleIn] Added %.2f lots — total now ~%.2f / %.2f target",
                  addLots, totalLots + addLots, g_TargetLots);
   else
      PrintFormat("[ScaleIn] FAILED to add %.2f lots — error %d", addLots, GetLastError());
}
...
// NOTE: kept exclusive ("h < InpTradingEndNY") per the v1.12 CRITICAL fix — this
// boundary exists specifically to stop new trades opening at the 10:00 bar. Widening
// it would let FVG Sell see the 9am candle, but at the cost of reopening exactly the
// bug that fix closed. Left alone pending explicit confirmation that's wanted.
...
// v1.83: decimal places needed to represent a lot step exactly (0.1 -> 1, 0.01 -> 2)
int LotStepDigits(double step)
{
   if(step <= 0) return 1;
...
// v1.84: auto-detect the minimum lot step from the broker name — no input needs to be
// set by hand. BlackBull only fills 0.1 increments (its reported step can't be trusted);
// Vault Markets and AvaTrade accept 0.01. InpMinLotStepOverride > 0 forces a specific
// value instead.
// v1.99: AvaTrade's ACCOUNT_COMPANY reports as "Ava Trade Markets Ltd." — two words,
// with a space — so the "avatrade" substring check never matched and every AvaTrade
// account was silently falling through to the 0.1 default. Confirmed live: lots were
// only ever landing on 0.1 increments (1.20, 1.10, ...) instead of true 0.01 precision.
// Now checks both the one-word and two-word spellings.
double GetMinLotStep()
{
   if(InpMinLotStepOverride > 0) return InpMinLotStepOverride;
   string company = AccountInfoString(ACCOUNT_COMPANY);
   StringToLower(company);
   if(StringFind(company, "vault") >= 0)     return 0.01;
   if(StringFind(company, "avatrade") >= 0)  return 0.01;
   if(StringFind(company, "ava trade") >= 0) return 0.01;
   return 0.1;
}
...
double CalcLotSize(double slPips)
{
   ...
   // enforce the auto-detected minimum lot step on top of the broker's own step
   // (BlackBull needs 0.1; Vault Markets gets 0.01 automatically — see GetMinLotStep)
   double effectiveStep = MathMax(step, GetMinLotStep());
   int    lotDigits     = LotStepDigits(effectiveStep);
   lots = NormalizeDouble(MathRound(lots / effectiveStep) * effectiveStep, lotDigits);
   ...
   {
      double freeMargin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
      double maxAffordable = NormalizeDouble(MathFloor((freeMargin * 0.9) / marginPerLot / effectiveStep) * effectiveStep, lotDigits);
...
// Returns the nearest bull→bear body transition whose open is ABOVE max(minPrice, AsianHigh).
// v1.82: filter raised to MathMax(minPrice, g_AsianHigh) — the Asian session already swept
// any pre-Asian level below the Asian High, so those levels are meaningless as TP targets.
// Only structural levels ABOVE the Asian High (unswept resistance) are valid.
double GetSTHigh(double minPrice, int lookback = 20)
{
   // v1.92: lim previously ignored the lookback parameter entirely and scanned nearly
   // the whole available price history, so with no qualifying recent swing high it could
   // return a swing from weeks earlier — producing absurd, meaningless RRR values (seen
   // live: RRR 16.45 against a TP from over a week prior). Bounded to match GetSTLowForSL.
   int lim = MathMin(lookback, iBars(_Symbol, PERIOD_H1) - 2);
   datetime asianStart = TodayMidnight() - (24 - InpAsianStartNY) * 3600;
   double minLevel = MathMax(minPrice, g_AsianHigh);
   ...
   for(int i = 1; i <= lim; i++)
   {
      ...
      if(IsBearishCandle(i) && IsBullishCandle(i + 1))
      {
         ...
         if(level > minLevel)
         {
            PrintFormat("[GetSTHigh] Bull→Bear above Asian High at bar[%d] (pre-Asian): %.5f (minLevel=%.5f)", i, level, minLevel);
            return level;
         }
      }
   }

   PrintFormat("[GetSTHigh] No valid STH found above Asian High (%.5f) before Asian session", minLevel);
   return 0;
}

// GetSTLow — TP for sell setups.
// Scans backwards (most-recent first) through bars BEFORE today's Asian session start.
// Returns the nearest bear→bull body transition whose open is BELOW min(maxPrice, AsianLow).
// v1.82: filter lowered to MathMin(maxPrice, g_AsianLow) — the Asian session already swept
// any pre-Asian level above the Asian Low, so those levels are meaningless as TP targets.
// Only structural levels BELOW the Asian Low (unswept support) are valid.
double GetSTLow(double maxPrice, int lookback = 20)
{
   // v1.92: lim previously ignored the lookback parameter entirely and scanned nearly
   // the whole available price history, so with no qualifying recent swing low it could
   // return a swing from weeks earlier — producing absurd, meaningless RRR values. Bounded
   // to match GetSTLowForSL/GetSTHighForSL, which already did this correctly.
   int lim = MathMin(lookback, iBars(_Symbol, PERIOD_H1) - 2);
   datetime asianStart = TodayMidnight() - (24 - InpAsianStartNY) * 3600;
   double maxLevel = MathMin(maxPrice, g_AsianLow);
   ...
   for(int i = 1; i <= lim; i++)
   {
      ...
      if(IsBullishCandle(i) && IsBearishCandle(i + 1))
      {
         ...
         if(level < maxLevel)
         {
            PrintFormat("[GetSTLow] Bear→Bull below Asian Low at bar[%d] (pre-Asian): %.5f (maxLevel=%.5f)", i, level, maxLevel);
            return level;
         }
      }
   }

   PrintFormat("[GetSTLow] No valid STL found below Asian Low (%.5f) before Asian session", maxLevel);
   return 0;
}
...
// v1.74: when a single bar breaks BOTH levels, bar close is used to determine direction
   ...
   for(int j = ArraySize(indices) - 1; j >= 0; j--)
   {
      ...
      if(brokeHigh && brokeLow)
      {
         ...
         if(!firstIsLow && !firstIsHigh)
         {
            ...
            {
               PrintFormat("[AsianBreak] Bar[%d] swept Low, closed above Asian High (%.2f) → SELL direction", i, barClose);
               firstIsHigh = true;
            }
            else if(barClose < g_AsianLow)
            {
               PrintFormat("[AsianBreak] Bar[%d] swept High, closed below Asian Low (%.2f) → BUY direction", i, barClose);
               firstIsLow = true;
            }
            else
            {
               PrintFormat("[AsianBreak] Bar[%d] close %.2f between levels — ambiguous → no trade", i, barClose);
               return 0;
            }
         }
         continue;
...
bool ScanBuySetups()
{
   ...
   // v1.65: FVG Buy restricted to London KZ only (02:00–06:00) after backtest showed
   // 56% WR London vs 31% NY.
   // v1.86: fixed the 5am candle being permanently unreadable (only becomes bar[1] at
   // the hr=6 tick, one hour past the old exclusive "hr < InpNYKillZoneNY" bound).
   // v1.87: NY-session restriction removed entirely — FVG Buy now scans the full
   // London + NY KZ window (02:00–10:00), same as FVG Sell's Priority 1 window below,
   // subsuming v1.86's fix (hr=6 is within this wider range too). Upper bound stays
   // exclusive ("< InpTradingEndNY") per the v1.12 CRITICAL fix on this exact boundary
   // — see IsInTradingWindow() — so the 9am candle is still not reachable here.
...
bool TryFVGAsianBuy()
{
   ...
   // v1.100: pull TP in InpTPBufferPips closer to entry than the structural target
   tp -= PipsToPrice(InpTPBufferPips);
   PrintFormat("[FVG_Asian_Buy] TP buffered %d pips closer to entry: %.5f", InpTPBufferPips, tp);
...
// FVG Buy (v1.98 rework)
// The gap candle's CLOSE clears the high of the candle right before it — see the scan
// loop below; the open doesn't matter, only where the gap candle settled. Entry:
//   RR >= 2.0 at current market price → enter now, wherever price is
//   1.5 <= RR < 2.0 → BuyLimit at a math-derived price giving exactly 1:2 RR (not tied
//                      to the FVG zone itself)
//   RR <  1.5 → skip
// SL = the wick of the candle right before the gap candle, + buffer.
// TP = nearest structural swing high (fallback to 3R/Asian High).
bool TryFVGBuy()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.93: gap check now uses candle 2's CLOSE, not its body-top/bottom (MathMax/Min of
      // open+close). A candle that opens back inside candle 1's range and only closes
      // beyond it (e.g. opens below, closes above) still leaves the zone above candle 1's
      // high untouched until close — the open isn't relevant to whether that zone got
      // filled. Only the close (where the candle actually settled) matters.
      double c3Close = iClose(_Symbol, PERIOD_H1, i);
      double c2High  = iHigh(_Symbol, PERIOD_H1, i + 1);

      if(c3Close > c2High)
      {
         fvgBar = i;
         PrintFormat("[FVG_Buy] Bullish FVG at bar[%d] %02d:00 ET — close %.2f > bar[%d] high %.2f (gap %.2f pts)",
                     i, bDt.hour, c3Close, i+1, c2High, c3Close - c2High);
         break;
      }
      else
      {
         PrintFormat("[FVG_Buy] bar[%d] %02d:00 ET: close=%.2f bar[%d] high=%.2f gap=%.2f — no FVG",
                     i, bDt.hour, c3Close, i+1, c2High, c3Close - c2High);
      }
   }
   ...
   // v1.98: Faulty bullish FVG redefined — only the two candles that form the gap are
   // checked now (the older "candle 1" used before is out of the picture entirely). If
   // the gap candle's own low dips below the low of the candle right before it, AND the
   // gap candle closed between 02:00-04:00 ET, the setup is skipped. That same dip at
   // 05:00 ET or later no longer disqualifies the FVG.
   double faultyLow    = iLow(_Symbol, PERIOD_H1, fvgBar);
   double priorBarLow  = iLow(_Symbol, PERIOD_H1, fvgBar + 1);
   MqlDateTime fvgDt;
   TimeToStruct(BarTimeNY(fvgBar), fvgDt);
   if(fvgDt.hour <= 4 && faultyLow < priorBarLow)
   {
      PrintFormat("[FVG_Buy] SKIP: faulty FVG — bar[%d] (%02d:00 ET) low %.2f below bar[%d] low %.2f",
                  fvgBar, fvgDt.hour, faultyLow, fvgBar + 1, priorBarLow);
      return false;
   }

   double sl = iLow(_Symbol, PERIOD_H1, fvgBar + 1) - PipsToPrice(InpFVGBuffer);

   // v1.96: entry reverted to the original 3-tier structure confirmed as correct — the
   // v1.90/v1.91 "wait for a retrace into the FVG zone" concept is removed entirely.
   //   RR >= 2.0 at market  → enter now, wherever price is
   //   1.5 <= RR < 2.0      → BuyLimit at a math-derived price giving exactly 1:2 RR
   //                          (entry12 below) — not tied to the FVG zone itself
   //   RR < 1.5             → skip
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   if(sl <= 0 || sl >= ask)
   { PrintFormat("[FVG_Buy] SKIP: invalid SL %.5f vs ask %.5f", sl, ask); return false; }
   ...
   if(tp <= ask)
   {
      ...
      PrintFormat("[FVG_Buy] No STH (or already reached) — fallback TP: 3R=%.5f AsianHigh=%.5f → using %.5f", tp3R, tpAsianHi, tp);
   }
   // v1.94: if every Asian session candle was bearish and Asian Low still broke, the whole
   // session was a one-way sell-off with no real resistance structure above; use the Asian
   // High as TP instead of a structural swing high.
   if(g_AllAsianBearish && g_AsianHigh > ask)
   {
      tp = g_AsianHigh;
      PrintFormat("[FVG_Buy] All-bear Asian — TP overridden to Asian High: %.5f", tp);
   }

   // v1.100: pull TP in InpTPBufferPips closer to entry than the structural target
   tp -= PipsToPrice(InpTPBufferPips);
   PrintFormat("[FVG_Buy] TP buffered %d pips closer to entry: %.5f", InpTPBufferPips, tp);
...
bool TryStraightBuy()
{
   ...
   // v1.100: pull TP in InpTPBufferPips closer to entry than the structural target
   tp -= PipsToPrice(InpTPBufferPips);
   PrintFormat("[Straight_Buy] TP buffered %d pips closer to entry: %.5f", InpTPBufferPips, tp);
...
bool ScanSellSetups()
{
   ...
   // NOTE: kept exclusive per the v1.12 CRITICAL fix on this same boundary — see
   // IsInTradingWindow(). The 9am candle stays unreachable here for the same reason.
   if(!triggered && hr >= InpLondonStartNY && hr < InpTradingEndNY)
...
bool TryFVGAsianSell()
{
   ...
   // v1.100: pull TP in InpTPBufferPips closer to entry than the structural target
   tp += PipsToPrice(InpTPBufferPips);
   PrintFormat("[FVG_Asian_Sell] TP buffered %d pips closer to entry: %.5f", InpTPBufferPips, tp);
...
// FVG Sell (v1.98 rework)
// The gap candle's CLOSE clears the low of the candle right before it — see the scan
// loop below; the open doesn't matter, only where the gap candle settled. Entry:
//   RR >= 2.0 at current market price → enter now, wherever price is
//   1.5 <= RR < 2.0 → SellLimit at a math-derived price giving exactly 1:2 RR (not tied
//                      to the FVG zone itself)
//   RR <  1.5 → skip
// SL = the wick of the candle right before the gap candle, + buffer.
// TP = nearest structural swing low (fallback to 3R/Asian Low).
bool TryFVGSell()
{
   if(g_DailyCount >= InpMaxDailyTrades) return false;
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.93: gap check now uses candle 2's CLOSE, not its body-top/bottom (MathMax/Min of
      // open+close). A candle that opens back inside candle 1's range and only closes
      // beyond it (e.g. opens above, closes below) still leaves the zone below candle 1's
      // low untouched until close — the open isn't relevant to whether that zone got
      // filled. Only the close (where the candle actually settled) matters.
      double c3Close = iClose(_Symbol, PERIOD_H1, i);
      double c2Low   = iLow(_Symbol, PERIOD_H1, i + 1);

      if(c3Close < c2Low)
      {
         fvgBar = i;
         PrintFormat("[FVG_Sell] Bearish FVG at bar[%d] %02d:00 ET — close %.2f < bar[%d] low %.2f (gap %.2f pts)",
                     i, bDt.hour, c3Close, i+1, c2Low, c2Low - c3Close);
         break;
      }
      else
      {
         PrintFormat("[FVG_Sell] bar[%d] %02d:00 ET: close=%.2f bar[%d] low=%.2f gap=%.2f — no FVG",
                     i, bDt.hour, c3Close, i+1, c2Low, c3Close - c2Low);
      }
   }
   ...
   // v1.98: Faulty bearish FVG redefined — only the two candles that form the gap are
   // checked now (the older "candle 1" used before is out of the picture entirely). If
   // the gap candle's own high climbs above the high of the candle right before it, AND
   // the gap candle closed between 02:00-04:00 ET, the setup is skipped. That same move
   // at 05:00 ET or later no longer disqualifies the FVG.
   double faultyHigh   = iHigh(_Symbol, PERIOD_H1, fvgBar);
   double priorBarHigh = iHigh(_Symbol, PERIOD_H1, fvgBar + 1);
   MqlDateTime fvgDt;
   TimeToStruct(BarTimeNY(fvgBar), fvgDt);
   if(fvgDt.hour <= 4 && faultyHigh > priorBarHigh)
   {
      PrintFormat("[FVG_Sell] SKIP: faulty FVG — bar[%d] (%02d:00 ET) high %.2f above bar[%d] high %.2f",
                  fvgBar, fvgDt.hour, faultyHigh, fvgBar + 1, priorBarHigh);
      return false;
   }
   ...
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(sl <= bid)
   { PrintFormat("[FVG_Sell] SKIP: invalid SL %.5f vs bid %.5f", sl, bid); return false; }
   ...
   if(tp <= 0 || tp >= bid)
   {
      ...
      PrintFormat("[FVG_Sell] No STL (or already reached) — fallback TP: 3R=%.5f AsianLow=%.5f → using %.5f", tp3R, tpAsianLo, tp);
   }
   ...
   // v1.100: pull TP in InpTPBufferPips closer to entry than the structural target
   tp += PipsToPrice(InpTPBufferPips);
   PrintFormat("[FVG_Sell] TP buffered %d pips closer to entry: %.5f", InpTPBufferPips, tp);
...
bool TryStraightSell()
{
   ...
   // v1.100: pull TP in InpTPBufferPips closer to entry than the structural target
   tp += PipsToPrice(InpTPBufferPips);
   PrintFormat("[Straight_Sell] TP buffered %d pips closer to entry: %.5f", InpTPBufferPips, tp);
