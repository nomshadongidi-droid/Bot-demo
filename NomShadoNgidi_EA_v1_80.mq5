...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.80        |
//+------------------------------------------------------------------+
//
...
//    Previously the scan only considered bars from 02:00 ET as the FVG candle (c3). When the
//    Asian Low/High break candle (01:00 ET) is itself the sweep-and-gap bar — i.e. it swept
//    the Asian Low then closed with its body above the 00:00 bar's high — the FVG was invisible
//    to the scan and the setup was missed entirely.
//    Fix: allow the 01:00 bar as a valid c3. Entry still fires only during London KZ (02:00+)
//    since ScanBuySetups/ScanSellSetups gate TryFVGBuy/TryFVGSell on hr >= 2.
//    SL for fvgBar at 01:00: iLow(bar[fvgBar+1]) = iLow(00:00 bar) — the pre-sweep
//    Asian session level, which is the correct structural invalidation point.
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.80"
...
int OnInit()
{
   ...
   PrintFormat("=== Nomshado Ngidi EA v1.80 Initialised ===");
...
bool TryFVGBuy()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      if(barDay < todayMid) break;
      if(bDt.hour < 1 || bDt.hour > 9) continue;

      // v1.76: FVG = body of candle 3 entirely above candle 2's HIGH
      // (candle 3 body bottom > candle 2 high — gap between c2 high and c3 body)
...
bool TryFVGSell()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.76: FVG = body of candle 3 entirely below candle 2's LOW
      // (candle 3 body top < candle 2 low — gap between c3 body and c2 low)
