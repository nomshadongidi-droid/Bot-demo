...
//|           Instrument: US30 / US_30 / US.30  |  Version 1.88        |
//+------------------------------------------------------------------+
//
...
#property description "MT5 EA — Nomshado Ngidi Trading Plan v1.88"
...
bool TryFVGBuy()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.81: FVG = body of candle 3 entirely above candle 2's BODY TOP (body-vs-body, ignores wicks)
      double c3BodyBot = MathMin(iOpen(_Symbol, PERIOD_H1, i), iClose(_Symbol, PERIOD_H1, i));
      double c2BodyTop = MathMax(iOpen(_Symbol, PERIOD_H1, i + 1), iClose(_Symbol, PERIOD_H1, i + 1));

      if(c3BodyBot > c2BodyTop)
      {
         fvgBar = i;
         PrintFormat("[FVG_Buy] Bullish FVG at bar[%d] %02d:00 ET — body bottom %.2f > bar[%d] body top %.2f (gap %.2f pts)",
                     i, bDt.hour, c3BodyBot, i+1, c2BodyTop, c3BodyBot - c2BodyTop);
         break;
      }
      else
      {
         PrintFormat("[FVG_Buy] bar[%d] %02d:00 ET: body bottom=%.2f bar[%d] body top=%.2f gap=%.2f — no FVG",
                     i, bDt.hour, c3BodyBot, i+1, c2BodyTop, c3BodyBot - c2BodyTop);
      }
   }
...
bool TryFVGSell()
{
   ...
   for(int i = 1; i < totalBars; i++)
   {
      ...
      // v1.81: FVG = body of candle 3 entirely below candle 2's BODY BOTTOM (body-vs-body, ignores wicks)
      double c3BodyTop = MathMax(iOpen(_Symbol, PERIOD_H1, i), iClose(_Symbol, PERIOD_H1, i));
      double c2BodyBot = MathMin(iOpen(_Symbol, PERIOD_H1, i + 1), iClose(_Symbol, PERIOD_H1, i + 1));

      if(c3BodyTop < c2BodyBot)
      {
         fvgBar = i;
         PrintFormat("[FVG_Sell] Bearish FVG at bar[%d] %02d:00 ET — body top %.2f < bar[%d] body bottom %.2f (gap %.2f pts)",
                     i, bDt.hour, c3BodyTop, i+1, c2BodyBot, c2BodyBot - c3BodyTop);
         break;
      }
      else
      {
         PrintFormat("[FVG_Sell] bar[%d] %02d:00 ET: body top=%.2f bar[%d] body bottom=%.2f gap=%.2f — no FVG",
                     i, bDt.hour, c3BodyTop, i+1, c2BodyBot, c3BodyTop - c2BodyBot);
      }
   }
